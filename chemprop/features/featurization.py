...
class MolGraph:
    ...
    def __init__(self, smiles: str, args: Namespace, prompt: bool):
        ...
        with open(f'./chemprop/data/{kg_name}/funcgroup.txt', "r") as f:
        ...
        self.fg2emb = pickle.load(open(f'initial/{kg_name}/fg2emb.pkl', 'rb'))
        self.rel2emb = pickle.load(open(f'initial/{kg_name}/rel2emb.pkl','rb'))
        self.ele2emb = pickle.load(open(f'initial/{kg_name}/ele2emb.pkl','rb'))

        mol = Chem.MolFromSmiles(smiles)
        self.f_fgs = self.match_fg(mol, args)
                ...
                for a2 in range(a1 + 1, self.n_atoms):
                    if a2 < self.n_real_atoms:
                        ...
                        if (self.atomic_nums[a1],self.atomic_nums[a2]) in rel2emb.keys():
                            f_bond = self.relation_features(self.atomic_nums[a1], self.atomic_nums[a2])
                        else:
                            continue      
    ...
    def ele_features(self, ele):
        fele = self.ele2emb[ele]
        return fele.tolist()

    def relation_features(self,e1,e2):
        frel = self.rel2emb[(e1,e2)]
        return frel.tolist()

    def match_fg(self, mol, args):
        fg_emb = [[1] * 133]
        pad_fg = [[0] * 133]
...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs, args: Namespace):
        ...
        for mol_graph in mol_graphs:
            ...
            f_fgs_starting_index.append(total_count)
            total_count += len(mol_graph.f_fgs)
        ...
        self.f_fgs_index = torch.LongTensor(f_fgs_starting_index)
    ...
    def get_components(self) -> Tuple[torch.FloatTensor, torch.FloatTensor,
        ...
        """
        return self.f_atoms, self.f_bonds, self.a2b, self.b2a, self.b2revb, self.a_scope, self.atom_num, self.fg_num, self.f_fgs, self.fg_scope, self.f_fgs_index
